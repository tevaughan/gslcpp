...
///
/// `vector_s` has storage by way of \ref gsl::v_stat, and most of the
...
struct vector_s: public v_iface<v_stat<S, T>> {
  ...
  template<typename OT> vector_s(v_iface<v_stat<0, OT>> const &ov) {
    memcpy(*this, ov);
  }
  ...
  /// @tparam OT  Type of other elements.
  /// @param ov  Reference to source-vector.
  template<typename OT> vector_s(v_iface<v_view<OT>> const &ov) {
    memcpy(*this, ov);
  }
