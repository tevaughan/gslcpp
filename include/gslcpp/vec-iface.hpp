...
template<vec D> struct vec_iface: public D {
  ...
  element_t &operator[](size_t i) { return data()[i * v().stride]; }
  ...
  element_t const *ptr(size_t i) const {
    return gsl_vector_const_ptr(&v(), i);
  }

  /// Set every element.
  /// @param x  Value to which each element should be set.
  void set_all(element_t const &x) { gsl_vector_set_all(&v(), x); }
  ...
  /// @return  TBD.
  int scale(element_t const &x) { return gsl_vector_scale(&v(), x); }

  /// Multiply scalar into this vector in place.
  /// @param x  Scalar to multiply into this.
  /// @return  Reference to this vector after modification.
  vec_iface &operator*=(element_t const &x) {
    scale(x);
    return *this;
  }
  ...
  /// @return  TBD.
  int add_constant(element_t const &x) {
